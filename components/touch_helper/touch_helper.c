...
void update_touches_task(void* pvParameters)
{
    ...
    while (1)
    {
        ...
        //ESP_LOGI("TOUCH", "touch_next_value: %d", touch_next_value);
        //ESP_LOGI("TOUCH", "touch_prev_value: %d", touch_prev_value);
        ...
        vTaskDelay(pdMS_TO_TICKS(TOUCH_UPDATE_TIME_MS));
    }
}
